...
export type EmojiPickerPanelProps = {
  ...
  /** Called after a choice (e.g. close parent popover). */
  onPicked?: () => void;
  /**
   * When true, root + grid use flex-1/min-h-0 so the scroll region fills a flex parent (tabs, popover).
   * When false, the grid uses a fixed max-height so scrolling works without a flex chain.
   */
  embedInFlex?: boolean;
  className?: string;
};
            ...
            {recentEntries.map((e) => (
              <button
                ...
                className={cn(
                  ...
                  value === e.emoji && 'ring-1 ring-kern-accent/40'
                )}
