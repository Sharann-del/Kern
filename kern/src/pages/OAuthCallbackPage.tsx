  ...
  useEffect(() => {
    ...
    (async () => {
      if (oauthError) {
        if (!cancelled) {
          setMessage(
            oauthErrorDescription?.replace(/\+/g, ' ') ?? `OAuth error: ${oauthError}`
          );
        }
        return;
      }
      ...
      if (provider === 'google') {
        ...
        const syncDaysBack = [30, 90, 365].includes(parsed) ? parsed : 90;
        const calendarId =
          sessionStorage.getItem('google_calendar_id')?.trim() || 'primary';
        const redirectUri = `${window.location.origin}/oauth/callback/google`;

        const { data, error, response } = await invokeAuthedEdgeFunction<unknown>('oauth-callback-google', {
          body: {
            code,
            collection_id: collectionId,
            redirect_uri: redirectUri,
